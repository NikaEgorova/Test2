...
bg2 = pygame.image.load('assets/bg1.png')
bg = bg1
pigeon_adventure = pygame.image.load('assets/pigeonadv.png')
...
seeds_group = pygame.sprite.Group()
exit_group = pygame.sprite.Group()
enemies_group = pygame.sprite.Group()
platform_group = pygame.sprite.Group()
bridge_group = pygame.sprite.Group()
groups = [water_group, lava_group, forest_group, seeds_group, enemies_group, exit_group, platform_group,
			bridge_group]
...
play_btn = Button(play, (90, 90), WIDTH//2 - WIDTH // 16, HEIGHT//2)
...
while running:
	...
	if main_menu:
		win.blit(pigeon_adventure, (WIDTH//3 - WIDTH//10, HEIGHT//4))
		...
		if not game_over and not game_won:
			...
			if pygame.sprite.spritecollide(player, seeds_group, True):
				sounds[0].play()
				cur_score += 1
				score += 1	
