...
pygame.mixer.music.load('sounds/bgrmusic.mp3')
pygame.mixer.music.set_volume(0.3)
pygame.mixer.music.play(-1)

seed_fx = pygame.mixer.Sound('sounds/pickupseed.wav')
seed_fx.set_volume(0.5)
jump_fx = pygame.mixer.Sound('sounds/jump.wav')
jump_fx.set_volume(0.5)
dead_fx = pygame.mixer.Sound('sounds/jump.wav')
dead_fx.set_volume(0.5)
sounds = [seed_fx, jump_fx, dead_fx]
		...
		for row in data:
			...
			for col in row:
				if col > 0:
					...
					if col == 17:
						# seed
						seed = Seeds(col_count * tile_size, row_count * tile_size)
						self.groups[3].add(seed)
					if col == 19:
						# water block
					...
					if col == 29:
						#cat
						cat = Cat(col_count * tile_size - 10, row_count * tile_size + tile_size // 8)
						self.groups[4].add(cat)


				col_count += 1
			row_count += 1

			seed = Seeds((WIDTH//tile_size - 3) * tile_size, tile_size // 2)
			self.groups[3].add(seed)
...
class Player:
	...
	def reset(self, win, pos, world, groups):
		...
		for i in range(1):
			img = pygame.image.load('player/pigeon1.png')
			img_right = pygame.transform.scale(img, (50,40))
...
class Seeds(pygame.sprite.Sprite):
	def __init__(self, x, y):
		...
		img = pygame.image.load('assets/sesame.png')
		self.image = pygame.transform.scale(img, (tile_size, tile_size))
		self.rect = self.image.get_rect()
		self.rect.x = x
		self.rect.y = y
...
class Cat(pygame.sprite.Sprite):
	def __init__(self, x, y):
		...
		self.img_left = pygame.transform.scale(img, (int(1.4*tile_size), 50))
		self.img_right = pygame.transform.flip(self.img_left, True, False)
...
def draw_text(win, text, pos):
	img = score_font.render(text, True, DARK)
	win.blit(img, pos)
